...
static uint32_t adpcm_converter(uint8_t * audio_file);
...
uint32_t adpcm_encode(uint8_t * audio_file)
{
    flags &= ~ADPCM_FLAG_NOISE_SHAPING;
    static_shaping_weight = 0.0;
    uint32_t ret_val = adpcm_converter(audio_file);
    if (ret_val != 0){
	return ret_val;
    }
...
static uint32_t adpcm_converter(uint8_t * input)
{
