...
#define DIRAM_ATTR __attribute__((section(".iram0.text")))
...
char DIRAM_ATTR fgetc2(uint8_t **fp)
{
    char ret = **fp;
    ++*fp;
    return ret;
...
int32_t DIRAM_ATTR getint(uint8_t **fp)
{
    int32_t c, i;
...
int16_t DIRAM_ATTR get_image(uint8_t **fp, int32_t *x_size, int32_t *y_size)
{
    char header[100];
    int temp;
...
void DIRAM_ATTR int_to_uint8_t(int32_t *r, uint8_t *in, int32_t size)
{
    int32_t i,
        max_r = r[0],
        min_r = r[0];
...
void DIRAM_ATTR setup_brightness_lut(uint8_t **bp, int32_t thresh, int32_t form)
{
    int32_t k;
    float temp;
...
void DIRAM_ATTR corner_draw(uint8_t *in, CORNER_LIST corner_list, int32_t x_size, int32_t drawing_mode)
{
    uint8_t *p;
    int32_t n = 0;
...
int16_t DIRAM_ATTR susan_corners_quick(uint8_t *in, int32_t *r, uint8_t *bp, int32_t max_no, CORNER_LIST corner_list, int32_t x_size, int32_t y_size)
{
    int32_t n, x, y, i, j;
    uint8_t *p, *cp;
...
int32_t DIRAM_ATTR susan_corners(uint8_t * in)
{
    	uint8_t *bp;
    	...
    	ret_val = get_image(&in, &x_size, &y_size);
    	if(ret_val < 0){
		return -1;
    	}

    	setup_brightness_lut(&bp, bt, 2);
    	ret_val = susan_corners_quick(in, g_r, bp, max_no_corners, corner_list, x_size, y_size);
    	if(ret_val < 0){
		return -1;
	}
    	corner_draw(in, corner_list, x_size, drawing_mode);
	int_to_uint8_t(g_r,in, 10);
