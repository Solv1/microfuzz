...
volatile uint32_t fuzzer_return;
 volatile cpuState_t reg_context;
volatile testCase_t * g_testCaseBuffer;
void (*cleanup_function)(void) = &execution_cleanup;

volatile testCase_t g_localPool[LOCAL_POOL_SIZE];


volatile uint8_t g_nSeeds; /*Used to be a size_t, we will never have more then 255 seeds at a time, makes host side easier with uint8_t*/

poolContext_t g_poolContext;
mutationContext_t g_mutContext ;

volatile uint32_t g_randomSeed ;

extern uint32_t g_coverageList[MAX_BLOCKS_PER_INPUT];
extern uint32_t g_covListIndex;
uint32_t * g_sutStartPtr  = 0;
uint8_t g_isIncreasing  = 0;
uint8_t g_isStale = 0;
volatile uint32_t g_iterations  = 0;
volatile uint8_t g_isReset  = 0;
uint8_t g_isRestore  = 0;
extern void * g_sutPtr;
...
int16_t fuzzer_setup(void * funPtr){
	...
	esp_task_wdt_deinit(); //Stop the WTD from resetting the board...
...
//        g_covListPtr = g_coverageList; //Used in ASM function to point to the coverage list
	...
	//asm volatile("break 0, 15\n\t");

	asm volatile("_jump_label:\n\t");
	srand(g_randomSeed);
	...
	#ifdef STD_LIBRARY
        	memset((uint32_t *)g_localPool, 0, sizeof(g_localPool ));
        	memset((uint32_t *)g_coverageList,0,sizeof(g_coverageList));
	#else
...
/*-------------------------Mutator Body--------------------------------*/
        ...
        for(i = posCur; i < g_testCaseBuffer->size; i++){
                testCase[i] = g_localPool[rndSeed].testCase[posNext];
                posNext++;
...
void dequeue_seed(void){
	...
	if (g_isStale) { //sync the cache.
		esp_cache_msync(g_localPool, sizeof(g_localPool), ESP_CACHE_MSYNC_FLAG_DIR_M2C);
		g_isStale = 0;
	}
...
/*	__asm volatile (
	...
	while(1) {
		...
		start_fuzz_time();
		target_harness((uint8_t *)g_testCaseBuffer->testCase, g_testCaseBuffer->size);
		end_fuzz_time();
